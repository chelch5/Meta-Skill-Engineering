...
| `scripts/meta_skill_studio/app.py` (`StudioCore`) | **Authoritative shared backend** | CLI, TUI, GUI, and Tauri should all converge on this workflow truth |
...
| Tauri application | Cross-platform convenience shell | Desktop UX and packaging path over the shared backend |
...
1. **Headless first:** every core workflow must remain reachable from the Python CLI without requiring Tauri, tkinter, or interactive TUI navigation.
2. **Shared workflow truth:** Tauri, TUI, and GUI may rename or restyle actions, but they must not invent a conflicting workflow contract.
...
4. **Desktop-specific stays layered:** Tauri can add bundling, menus, dashboards, and native shell affordances, but the workflow semantics belong to the shared Python backend and documented support scripts.
...
- `npm run tauri -- dev` for desktop shell usage

## Explicit non-goal

The Tauri shell is **not** allowed to become the only place where create/improve/evaluate/library-management workflows are fully real. If a workflow exists only in Tauri, the platform is incomplete.
